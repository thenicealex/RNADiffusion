...
class SegmentationUnet2DCondition(nn.Module):
    ...
    def __init__(
        ...
        if learned_time_emb:
            self.time_pos_emb = LearnedSinusoidalPosEmb(
                dim, scale=1.0, flip_sin_to_cos=False
            )
        else:
            self.time_pos_emb = SinusoidalPosEmb(
                dim, num_steps=num_steps, rescale_steps=4000, flip_sin_to_cos=False
            )
        ...
        self.fm_cond_2 = nn.Sequential(nn.Linear(600, 64), Mish(), nn.Linear(64, 8))
            ...
            self.downs.append(
                nn.ModuleList(
                    [
                        ...
                        (
                            Downsample_SP_conv(self.cond_dim)
                            if not is_last
                            else nn.Identity()
                        ),
                    ]
        ...
        out_dim = num_classes
        self.res_conv = ResnetBlock(
            dim, dim, time_emb_dim=dim, cond_dim=self.cond_dim, groups=8
        )
    ...
    def forward(self, time, x, esm_condition, seq_encoding):
        ...
        cond = None
        esm_embedding = self.fm_cond_1(esm_condition["embedding"]).permute(0, 2, 1)
        esm_attention_map = self.fm_cond_2(
            esm_condition["attention"].permute(0, 2, 3, 1)
        ).permute(0, 3, 1, 2)
        cond_L = esm_embedding.size(-1)

        esm_out_cat = torch.cat(
            [
                esm_embedding.unsqueeze(-1).repeat(1, 1, 1, cond_L),
                esm_embedding.unsqueeze(-2).repeat(1, 1, cond_L, 1),
            ],
            dim=1,
        )
        ...
        x = self.x_mlp(
            torch.cat([x, esm_out_cat, esm_attention_map, seq_out_cat], dim=1).permute(
                0, 2, 3, 1
            )
        ).permute(0, 3, 1, 2)
        ...
        for ind, (resnet1, resnet2, attn, downsample, cond_downsample) in enumerate(
            self.downs
        ):
            x = resnet1(x, t, cond)
            x = self.dropout(x)
...
if __name__ == "__main__":
    ...
    print(len(fm_cond))
    model = SegmentationUnet2DCondition(
        2, 8, 8, 1000, learned_time_emb=True, cat_cond=True
    )
