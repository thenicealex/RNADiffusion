...
class ContactPredictionHead(nn.Module):
    ...
    def accumulate_forward(self, tokens, attentions, layer_idx):
        # remove eos token attentions
        if self.append_eos:
            eos_mask = tokens.ne(self.eos_idx).to(attentions)
            eos_mask = eos_mask.unsqueeze(1) * eos_mask.unsqueeze(2)
        ...
        attentions = attentions.permute(0, 2, 3, 1)
        # return self.activation(self.regression(attentions).squeeze(3))
        num_head = attentions.shape[-1]
        weight = self.regression.weight[:, num_head*layer_idx:num_head*(layer_idx+1)]
        attentions = torch.einsum('btlc,oc->btlo', attentions, weight)
        return attentions.squeeze(-1)
