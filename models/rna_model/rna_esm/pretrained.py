...
def load_hub_workaround(url):
    try:
        data = torch.hub.load_state_dict_from_url(
            ...
            progress=False,
            map_location="cpu",
            model_dir=str(root_directory / "data"),
        )
    except RuntimeError:
...
def _load_model_and_alphabet_core_v1(model_data):
    import rna_esm  # since esm.inverse_folding is imported below, you actually have to re-import esm here

    alphabet = rna_esm.Alphabet.from_architecture(model_data["args"].arch)
    ...
    if model_data["args"].arch == "roberta_large":
        ...
        model_type = rna_esm.MSATransformer

    elif "invariant_gvp" in model_data["args"].arch:
        import rna_esm.inverse_folding

        model_type = rna_esm.inverse_folding.gvp_transformer.GVPTransformerModel
...
def esmfold_v0():
    ...
    """
    import rna_esm.esmfold.v1.pretrained

    return rna_esm.esmfold.v1.pretrained.esmfold_v0()
...
def esmfold_v1():
    ...
    return rna_esm.esmfold.v1.pretrained.esmfold_v1()
