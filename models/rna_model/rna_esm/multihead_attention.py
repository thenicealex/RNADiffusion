...
from models.rna_model.rna_esm.rotary_embedding import RotaryEmbedding
...
class MultiheadAttention(nn.Module):
    ...
    def __init__(
        ...
        use_rotary_embeddings: bool = False,    # True
    ):
        super().__init__()
        self.embed_dim = embed_dim
        self.kdim = kdim if kdim is not None else embed_dim
        self.vdim = vdim if vdim is not None else embed_dim
        self.qkv_same_dim = self.kdim == embed_dim and self.vdim == embed_dim   # True
    ...
    def forward(
        ...
        q = q.contiguous().view(tgt_len, bsz * self.num_heads, self.head_dim).transpose(0, 1)   # (B*NH, T, HD)
        if k is not None:
        ...
        attn_weights_float = F.softmax(attn_weights, dim=-1, dtype=torch.float32)  # type: ignore
        ...
        assert v is not None
        attn = torch.bmm(attn_probs, v)     #  (B*NH, T, T) * (B*NH, T, HD) => (B*NH, T, HD)
...
class PerformerAttention(MultiheadAttention):
    ...
    def forward(
        ...
        seqlen, bsz, embed_dim = query.size()
        ...
        attn = rearrange(attn, "b h t d -> t b (h d)")  # (B, NH, T, HD) => (T, B, D)
        attn = self.out_proj(attn)

        if need_weights or need_head_weights:
            v_pos = torch.eye(seqlen, dtype=v.dtype, device=v.device)[
                None, None
            ].repeat(bsz, self.num_heads, 1, 1)     # (B, NH, T, T)
