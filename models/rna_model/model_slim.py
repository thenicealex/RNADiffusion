...
from models.rna_model.evo.metrics import lmdata_compute_precisions
from models.rna_model.evo.tensor import symmetrize, apc

# from modules import (
from models.rna_model.modules_slim import (
    TransformerLayer,
    PKMLayer,
...
from models.rna_model.rna_esm.modules import (
    ESM1bLayerNorm,
...
from models.rna_model.lr_schedulers import get
from models.rna_model.product_key_memory import PKM
from models.rna_model.dataset import TRRosettaContactDataset
...
class ESM2(BaseProteinModel):
    ...
    def forward(
        ...
        if self.token_dropout:
            ...
            mask_ratio_observed = (tokens == self.vocab.mask_idx).sum(-1).to(
                x.dtype
            ) / src_lengths
    ...
    def slim_inference(self, tokens):
        ...
        B, T = x.shape[:2]
        contact_scores = torch.zeros([B, T - 2, T - 2]).to(x.device)
        ...
        for layer_idx, layer in enumerate(self.layers):
            ...
            contact_scores += self.contact_head.accumulate_forward(
                tokens, attn, layer_idx
            )
        ...
        result["embeddings"] = self.embed_tokens(tokens)
    ...
    def from_esm(
        ...
        esm_pretrained_dict = {
            key: value for key, value in esm_state_dict.items() if (key in model_dict)
        }
