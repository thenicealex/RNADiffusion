...
def prediction(config, model, base_info, tokens, seq_encoding_pad, seq_length, set_max_len):
    ...
    with torch.no_grad():
        ...
        candidate_seeds = np.arange(1970, 2024)
        ...
        for seed_ind in select_seeds:
            ...
            pred_x0, _ = model.sample(batch_size, base_info, tokens,
...
if __name__ == '__main__':
    ...
    base_info, tokens, seq_encoding_pad, seq_length, name_list, set_max_len, seq_list, seq_len_list = \
        get_data(join(ROOT_PATH, f'predict_data/{config.predict_data}'), alphabet)

    predict_results_list = prediction(config, model, base_info, tokens, seq_encoding_pad, seq_length, set_max_len)
