...
def get_data(file_path, alphabet):
    ...
    base_info = list(map(lambda x: get_data_fcn(x[0], x[1], set_max_len),
                          zip(seq_encoding_pad_list, seq_len_list)))

    seq_encoding_pad = torch.tensor(np.stack(seq_encoding_pad_list, axis=0)).float()
    base_info = torch.tensor(np.stack(base_info, axis=0)).float()
    seq_length = torch.tensor(seq_len_list).long()
    tokens = generate_token_batch(alphabet, seq_list)
    return base_info, tokens, seq_encoding_pad, seq_length, name_list, set_max_len, seq_list, seq_len_list
...
def get_data_fcn(data_seq, data_length, set_length):
    ...
    base_info = np.concatenate((data_fcn, data_fcn_1), axis=0)
...
if __name__ == '__main__':
    ...
    base_info, tokens, seq_encoding_pad_list, seq_length, name_list, set_max_len = \
